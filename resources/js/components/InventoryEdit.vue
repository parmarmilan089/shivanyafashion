<template>
    <div>
            ...
            <form @submit.prevent="handleSubmit">
                ...
                <div class="card px-sm-4 px-3 py-3 my-3">
                    <div class="row">
                        <div class="col-md-6 col-lg-3">
                            <div class="input-group input-group-outline my-2">
                                <select v-model="selectedCategoryId" class="form-control">
                                    ...
                                    <option v-for="cat in categories.filter(c => c.category_type === 0)" :key="cat.id"
                                        :value="cat.id">
                                        {{ cat.name }}
                                    </option>
                                </select>
                        ...
                        <div class="col-md-6 col-lg-3">
                            <div class="input-group input-group-outline my-2">
                                <select v-model="selectedSubcategoryId" class="form-control"
                        ...
                        <div class="col-md-6 col-lg-3">
                            <div class="input-group input-group-outline my-2">
                                <select v-model="selectedSubsubcategoryId" class="form-control"
                                    :disabled="!selectedSubcategoryId">
                                    <option value="">Select Sub-subcategory</option>
                                    <option v-for="subsub in filteredSubsubcategories" :key="subsub.id"
                                        :value="subsub.id">
                                        {{ subsub.name }}
                                    </option>
                                </select>
                ...
                <div class="card px-3 py-3">
                    ...
                    <div v-for="(variant, vIndex) in form.variants" :key="vIndex"
                        class="variant-block border p-3 mb-4 bg-white">
                        <div class="d-flex justify-content-between align-items-start flex-wrap gap-2">
                            ...
                            <div class="d-flex gap-2 mt-4">
                                <button type="button" class="btn btn-sm btn-dark" @click="addSizeRow(vIndex)">Add
                                    Size</button>
                        ...
                        <div class="row mt-3">
                            <div class="col-md-6 my-2">
                                <div class="file-div">
                                    ...
                                    <input type="file" class="form-control file-input w-100" accept="image/*"
                                        @change="handleVariantMainImage(vIndex, $event)" />
                                    ...
                                    <div v-if="variant.main_image">
                                        <img :src="variant.main_image_preview ? variant.main_image_preview : (typeof variant.main_image === 'string' ? '/storage/' + variant.main_image : '')"
                            ...
                            <div class="col-md-6 my-2">
                                <div class="file-div">
                                    ...
                                    <input type="file" class="form-control file-input w-100" accept="image/*" multiple
                                        @change="handleVariantGalleryImages(vIndex, $event)" />
                                    <div class="file-info mt-2">
                                        <small class="text-muted">
                                            <i class="material-icons" style="font-size: 14px;">info</i>
                                            Max file size: 2MB | Supported: JPEG, PNG, GIF, WebP
                                    ...
                                    <div v-if="getCombinedGalleryImages(vIndex).length > 0" class="gallery-container">
                                        <div v-for="(img, index) in getCombinedGalleryImages(vIndex)" :key="img.id"
                                            style="display: inline-block; margin: 5px; position: relative;">
                                            <img :src="img.url" :alt="img.alt"
                                                style="max-width: 80px; height: 80px; object-fit: cover; border-radius: 8px;" />
                                            <button type="button" @click="removeGalleryImage(vIndex, index, img.type)"
                                                class="btn btn-sm btn-danger gallery-remove-btn" title="Remove image">
                                                ×
                                            </button>
                        ...
                        <div class="mt-3">
                            ...
                            <div v-for="(size, sizeIndex) in variant.sizes" :key="sizeIndex" class="row mb-2">
                                <div class="col-md-2">
                                    <div class="input-group input-group-outline my-2">
                                        <select v-model="size.size_id" class="form-control">
                                            ...
                                            <option v-for="sz in sizes" :value="sz.id"
                                                :disabled="isSizeUsedInVariant(sz.id, vIndex)">
                                                {{ sz.name }}
                                            </option>
                                        </select>
                                    </div>
                                ...
                                <div class="col-md-2">
                                    <div class="input-group input-group-outline my-2">
                                        <input v-model="size.price" class="form-control" placeholder="Price"
                                            @input="autoFillPrice(vIndex, sizeIndex, $event.target.value)" />
                                        <div v-if="size.priceError" class="error-message">{{ size.priceError }}</div>
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="input-group input-group-outline my-2">
                                        <input v-model="size.sale_price" class="form-control"
                                            :class="{ 'is-invalid': size.salePriceError }" placeholder="Sale Price"
                                            @input="autoFillSalePrice(vIndex, sizeIndex, $event.target.value)" />
                                        <div v-if="size.salePriceError" class="error-message">{{ size.salePriceError }}
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="input-group input-group-outline my-2">
                                        <input v-model="size.stock" class="form-control" placeholder="Stock"
                                            @input="autoFillStock(vIndex, sizeIndex, $event.target.value)" />
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="input-group input-group-outline my-2">
                                        <input v-model="size.sale_start" type="date" class="form-control"
                                            @change="autoFillSaleStart(vIndex, sizeIndex, $event.target.value)" />
                                    </div>
                                </div>
                                <div class="col-md-2 d-flex gap-2">
                                    <div class="input-group input-group-outline my-2">
                                        <input v-model="size.sale_end" type="date" class="form-control"
                                            @change="autoFillSaleEnd(vIndex, sizeIndex, $event.target.value)" />
                                    </div>
                                    <button type="button" @click="removeSizeRow(vIndex, sizeIndex)"
        ...
        <div v-if="showCropper" class="cropper-modal">
            <div class="modal-box">
                <h2>Crop Image</h2>
                <Cropper ref="cropper" :src="cropperImage" :stencil-props="{ aspectRatio: 180 / 253 }" :auto-zoom="true"
                    :auto-crop="true" class="cropper" />
                <div class="actions">
                    <button class="cancel" @click="cancelCrop">Cancel</button>
                    <button class="save" @click="cropImage">Crop & Save</button>
                </div>
...
export default {
    ...
    data() {
        return {
            ...
            galleryCropQueue: [],
            cropperType: null, // 'main' or 'gallery'
        };
    },
    computed: {
    ...
    methods: {
        ...
        handleVariantMainImage(variantIndex, event) {
            ...
            this.cropperType = 'main'; // ✅ THIS IS REQUIRED
            this.showCropper = true;

            // Store the input so we can reset it after cropping
            this.lastUsedFileInput = fileInput;
        },
        cropAndSaveImage() {
            ...
            if (croppedImage) {
                this.variants[this.cropperVariantIndex].main_image_preview = croppedImage;
            }

            // Hide modal and clear temp data
            this.showCropper = false;
            this.cropperImage = null;
            this.cropperVariantIndex = null;

            // Reset file input
            if (this.lastUsedFileInput) {
                this.lastUsedFileInput.value = '';
                this.lastUsedFileInput = null;
            }
        },
        cropImage() {
            ...
            resizedCanvas.toBlob(blob => {
                ...
                if (this.cropperType === 'main') {
                    ...
                    Swal.fire({
                        title: 'Image Cropped',
                        text: 'Main image cropped and resized to 720x1012!',
                        icon: 'success',
                        timer: 1500,
                        showConfirmButton: false
                    });
                    ...
                    // Push the cropped file and preview
                    this.form.variants[variantIndex].gallery_images.push(file);
                    this.form.variants[variantIndex].newGalleryImages.push({
                        id: `new-${variantIndex}-${Date.now()}`,
                        url: previewUrl,
                        file: file,
                        type: 'new'
                    });
                    ...
                    if (this.galleryCropQueue.length > 0) {
                        ...
                        this.cropperImage = URL.createObjectURL(nextFile);
                    } else {
                        Swal.fire({
        ...
        handleVariantGalleryImages(variantIndex, event) {
            ...
            this.cropperVariantIndex = variantIndex;
            this.cropperType = 'gallery';

            // Start with first file
            const firstFile = this.galleryCropQueue.shift();
            this.cropperImage = URL.createObjectURL(firstFile);
            this.showCropper = true;
        },
