...
@section('front-content')
    ...
    <section class="hero-banner-section w-100">
        <div class="w-100">
            <div class="position-relative overflow-hidden banner-items"
                style="
            ...
            <div class="main-categories-card d-flex items-start gap-3 gap-lg-4 overflow-auto scroll-1">
                @foreach ($categorys as $category)
                    <a href="{{ url('category/' . $category['slug']) }}"
                        class="categories-card overflow-hidden text-decoration-none">
                        <!-- Category Image -->
                        <div class="categories-card-img w-90 overflow-hidden flex-1">
                            <img src="{{ asset($category['image']) }}" class="w-100 h-100 object-cover"
                                alt="{{ $category['name'] }}">
                        </div>
                        <!-- Category Name -->
                        <div class="p-3">
                        ...
                        <div class="product-card" data-product-id="{{ $product->id }}">
                            <div class="position-relative product-items-img">
                                @php
                                    ...
                                    $gallery =
                                        $firstVariant && $firstVariant->gallery_images
                                ...
                                @if ($hasGallery)
                                    <div id="carousel-{{ $product->id }}" class="carousel slide" data-bs-ride="carousel">
                                        <div class="carousel-inner" id="carousel-inner-{{ $product->id }}">
                                ...
                                @elseif($firstVariant && $firstVariant->main_image)
                                    <img src="{{ asset('storage/' . $firstVariant->main_image) }}"
                                        class="w-100 h-100 object-fit-cover" alt="{{ $product['name'] }}">
                            ...
                            @if ($variantsByColor->count() > 1)
                                    <div class="color-selection mb-1">
                                        <div class="color-options d-flex gap-2 flex-wrap justify-content-center">
                                            @foreach ($variantsByColor as $colorId => $colorVariants)
                                                ...
                                                @endphp
                                                @php
                                                    $colorGallery = $colorVariant->gallery_images
                                                        ? $colorVariant->gallery_images
                                                        : $relatedProduct->gallery_images ?? [];
                                                    $colorGalleryJson = json_encode($colorGallery);
                                                    $colorImage = $colorVariant->main_image
                                                        ? asset('storage/' . $colorVariant->main_image)
                                ...
                                @endif
                            <div class="d-flex flex-column justify-content-between gap-3 p-4 pt-0 text-center ">
                                <div>
                                    <h6 class="product-title mb-3">{{ Str::limit($product['name'], 20) }}</h6>
    ...
    <script>
        window.selectColor = function(btn, productId) {
            ...
            if (card) {
                const colorBtns = card.querySelectorAll('.color-option');
            ...
            if (carouselInner && Array.isArray(gallery)) {
                let html = '';
                gallery.forEach((img, idx) => {
                    html += `<div class="carousel-item${idx === 0 ? ' active' : ''}">
                    <img src="/storage/${img}" class="w-100 h-100 object-fit-cover" alt="">
                </div>`;
                });
