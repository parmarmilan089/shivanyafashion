            ...
            <tr>
                ...
                <div class="d-flex flex-column gap-2 mt-2">
                  ...
                  <a href="{{ route('admin.product.edit', $product->id) }}">
                    <div class="d-flex align-items-center gap-2">
                      <img src="{{ asset('storage/' . $product->image) }}" alt="Product Image" style="width: 40px; height: 40px; object-fit: cover; border-radius: 5px;">
                      <div>
              ...
              <td>
                <div class="d-flex flex-column gap-2">
                  <form action="{{ route('admin.order.updateStatus', $order->id) }}" method="POST" id="status-form-{{ $order->id }}">
                    @csrf
                    @method('PATCH')
                    <select name="order_status" onchange="toggleShippingInput(this, {{ $order->id }})" class="form-select form-select-sm">
                      <option value="RTO-Return" {{ $order->order_status == 'RTO-Return' ? 'selected' : '' }}>RTO-Return</option>
                      <option value="Wrong-RTO-Return" {{ $order->order_status == 'Wrong-RTO-Return' ? 'selected' : '' }}>Wrong-RTO-Return</option>
                      <option value="Wrong-Return" {{ $order->order_status == 'Wrong-Return' ? 'selected' : '' }}>Wrong-Return</option>
                      <option value="Missing-Return" {{ $order->order_status == 'Missing-Return' ? 'selected' : '' }}>Missing-Return</option>
